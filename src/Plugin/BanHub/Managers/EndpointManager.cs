...
public class EndpointManager
{
    ...
    public async Task<bool> CreateOrUpdateCommunityAsync(CreateOrUpdateCommunitySlim communitySlim) =>
        await _community.CreateOrUpdateCommunityAsync(communitySlim);
    ...
    public async Task OnStart(CreateOrUpdateServerCommandSlim server)
    {
    ...
    public async Task OnJoin(EFClient player)
    {
        ...
        var isBanned = await _playerService.IsPlayerBannedAsync(new IsPlayerBannedCommandSlim
        {
            Identity = identity,
    ...
    private async Task<bool> CreateOrUpdatePlayerAsync(EFClient player)
    {
        ...
        var createOrUpdate = new CreateOrUpdatePlayerNotificationSlim
        {
    ...
    public async Task HandleChatMessageAsync(ClientMessageEvent messageEvent, CancellationToken token)
    {
        ...
        try
        {
            ...
            var communityMessages = new AddCommunityChatMessagesNotificationSlim
            {
                CommunityGuid = _communitySlim.CommunityGuid,
        ...
        string reason, DateTimeOffset? expiration = null, PenaltyScope? scope = null)
    {
        ...
        var penaltyDto = new AddPlayerPenaltyCommandSlim
        {
    ...
    {
        var penalty = new AddPlayerPenaltyEvidenceCommandSlim
        {
            PenaltyGuid = guid,
            Evidence = evidence,
