...
using Server = Data.Domains.Server;
...
public class EndpointManager
{
    ...
    public EndpointManager(BanHubConfiguration banHubConfiguration, Instance instanceMeta,
    ...
    public async Task<bool> UpdateInstance(Instance instance) => await _instance.PostInstance(instance);

    public async Task<bool> IsInstanceActive(Instance instance) =>
        await _instance.IsInstanceActive(instance.InstanceGuid);

    private Player ClientToEntity(EFClient client)
    {
        var entity = new Player
        {
            Identity = $"{client.GuidString}:{client.GameName.ToString()}",
            Alias = new Alias
            {
    ...
    public async Task OnStart(Server server)
    {
    ...
    public async Task OnJoin(EFClient player)
    {
        ...
        if (!_instanceMeta.Active) return;
        ...
        var isBanned = await _entity.IsPlayerBannedAsync(identity);
        ...
        string reason, TimeSpan? duration = null, PenaltyScope? scope = null, string? evidence = null)
    {
        ...
        var penaltyDto = new Penalty
        {
            ...
            InstanceGuid = _instanceMeta.InstanceGuid,
            AdminIdentity = adminEntity.Identity,
            TargetIdentity = targetEntity.Identity
        };
    ...
    public async Task<string?> GenerateToken(EFClient client)
    {
        ...
        return await _entity.GetTokenAsync(entity.Identity);
    }
}
