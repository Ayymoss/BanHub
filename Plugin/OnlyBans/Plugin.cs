...
public class Plugin : IPluginV2
{
    ...
    public Plugin(Instance instance, HeartBeatManager heartBeatManager, EndpointManager endpointManager, BanHubConfiguration config,
    ...
    public static void RegisterDependencies(IServiceCollection serviceCollection)
    {
        ...
        serviceCollection.AddSingleton(new Instance());
    ...
    {
        var serverDto = new Data.Domains.Server
        {
    ...
    private async Task OnLoad(IManager manager, CancellationToken arg2)
    {
        ...
        _instance.InstanceGuid = Guid.Parse(_appConfig.Id);
        _instance.InstanceIp = manager.ExternalIPAddress;
        _instance.ApiKey = _config.ApiKey;

        // We need a copy of this since we don't really want the other values being sent with each request.
        var instanceCopy = new Instance
        {
            InstanceGuid = _instance.InstanceGuid,
            InstanceIp = _instance.InstanceIp,
        ...
        InstanceActive = await _endpointManager.IsInstanceActive(_instance);
        _instance.Active = InstanceActive;
